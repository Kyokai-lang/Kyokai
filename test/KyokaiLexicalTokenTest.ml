...
*)
open OUnit2
open Austral_core.KyokaiLexicalToken
...
let test_grammar_reserved_words _ =
  let source =
    "additional_invariant assumes below bit bitrecord bits borrows cleanup compile_error " ^
    "contract covers default drop evidence exports field fn forbids from layout lifetime " ^
    "maps_failure method module_invariant owns preserves produce qed reentrancy requires " ^
    "reserved target threading timeout to transfers wait wake"
  in
...
let test_contextual_words_remain_identifiers _ =
  assert_equal [Identifier; Identifier; Identifier; Eof] (kinds "result old ignore")
...
let test_scan_prepared_entry_shebang _ =
  ...
  match Austral_core.KyokaiSourceText.prepare Austral_core.KyokaiSourceText.ExecutableEntryBody source with
  ...
  | Ok prepared ->
     ...
     | Ok (keyword :: _) ->
        ...
        assert_equal 2 keyword.span.start_line;
        assert_equal (Austral_core.KyokaiSourceText.token_start_byte prepared) keyword.span.start_byte
