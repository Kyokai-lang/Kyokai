...
- **Kyokai carries an explicit formalization roadmap instead of pretending prose alone closes the soundness story** — the project treats the core calculus as part of the language plan, not as optional future polish.
  **Rules**:
  ...
  5. Lean 4 is the selected proof assistant for Kyokai mechanization. Elan pins the Lean toolchain and Lake builds checked artifacts. Whole-core mechanization remains after paper-core stabilization and self-hosting progress; narrow early Lean artifacts can check isolated high-risk lemmas without upgrading the broader theorem claim.
  6. The public calculus artifacts for this effort are `kyokaicalculus/scope.md`, `kyokaicalculus/claim-tiers.md`, `kyokaicalculus/syntax-and-statics.md`, `kyokaicalculus/dynamics.md`, `kyokaicalculus/lemmas.md`, `kyokaicalculus/paper-proof.md`, `kyokaicalculus/theorem-assembly.md`, the derivation packages, and `kyokaicalculus/mechanization-plan.md`. They record the theorem scope, maintained paper-proof status, proof obligations, exclusions, and mechanization plan.
  ...
  **[STAGE: DECIDED_CORE_SEMANTICS | D143/D241 -> phased formalization plan; sequential `lambda_K-seq` is `paper-proven`; Lean 4 is selected for mechanized proof after self-hosting]**
...
### D526: ProofTrace Evidence Graph, Code Markers, And Generated Public Status

Kyokai maintains a public `ProofTrace` evidence graph. Every spec-relevant semantic, toolchain, standard-library, backend, unsafe, conformance, and proof boundary has a stable record ID in `kyokaiproofstatus.toml`.

A `ProofTrace` record keeps these axes separate:

- `spec_status`: whether normative text exists for the registered scope;
- `implementation_status`: whether code is absent, planned, inherited bootstrap code, a prototype, or implemented Kyokai behavior;
- `conformance_status`: whether executable conformance coverage is absent, planned, or present;
- `proof_status`: the evidence tier from `kyokaicalculus/claim-tiers.md`;
- `proof_required`: whether the scope requires proof work.

A record with `proof_required = false` states one closed reason category: `tooling-service-behavior`, `workflow-only`, `documentation-only`, `infrastructure-only`, `bootstrap-harness`, or `generated-artifact-plumbing`. This exemption means that a theorem is not the acceptance criterion for that boundary. It does not waive implementation, testing, conformance, security, or review requirements.

Every registered record names its scope, owner, applicable artifacts, and exclusions. Normative spec chapters carry chapter-level `ProofTrace` markers. Maintained code, test-harness, and proof boundaries carry language-appropriate comments of the form `kyokai:prooftrace id=<record-id>`. A helper function needs a marker only when it is itself the maintained spec-relevant boundary. A boundary marker covers subordinate implementation details until a narrower boundary receives its own record.
