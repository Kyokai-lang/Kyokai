...
| --- | --- | --- | --- |
| Project identity | Kyokai uses `Kyokai.*`, one `.kyo` source file per module, `.koi` generated interfaces, Kyokai package manifests, and Kyokai-owned terminology. | `language/00-introduction.md`, `language/04-modules-and-visibility.md`, `toolchain/02-module-resolution-and-koi.md` | D1, D5, D52, D78, D537 |
...
| Syntax | Kyokai changes terminators, borrow spelling, pattern forms, fallible control flow, generics, records, comments, and replaces Austral's two-file `.aui`/`.aum` modules with one `.kyo` source file, per-declaration `public`/`internal` visibility, and an `opaque` representation modifier. | `language/02-lexical-syntax.md`, `language/03-grammar.md`, `language/04-modules-and-visibility.md`, `language/05-declarations.md` | D7b-D19a, D35-D43, D52, D63, D537, D538, D539 |
...
| Unsafe/FFI/backend | Kyokai specifies unsafe contracts, C ABI, dynamic loading, one generated-C backend, admitted C toolchains, volatile/MMIO, source mapping, no language UB, and a public explanation of rejected backend architectures. | `language/16-unsafe-ffi-and-abi.md`, `language/17-memory-layout-and-backend-contract.md`, `rationale/09-backend-choice.md` | D20, D73, D113a-D113b, D139, D228, D245, D530-D536 |
