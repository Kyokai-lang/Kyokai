...
The assembled specification has five roles: language semantics, observable toolchain contracts, standard-library contracts, rationale, and official-project contracts. Appendices carry source history and the full documentation license. The file tree keeps those roles distinct so that an implementation rule does not masquerade as language semantics, but the reader receives one specification.

The core language specification defines source syntax, static checking, types, ownership, borrowing, evaluation, control flow, capabilities, contracts, unsafe code, FFI, runtime failure, layout, and backend-observable language behavior. The toolchain specification defines manifests, packages, workspaces, build artifacts, the CLI, diagnostics, formatting, LSP behavior, tests, documentation generation, target support, reproducibility, and related tool behavior. The standard-library contract specification defines public API families, admission requirements, contracts, edge cases, allocation behavior, capability requirements, platform boundaries, and test obligations.

> Trace: D85, D86, D152, D229
> Covers: Kyokai assembles language, toolchain, standard-library, rationale, official-project, and appendix material into one specification while keeping their authority roles distinct; standard-library APIs require explicit semantic contract fields, the stdlib is a core project commitment, and stdlib admission requires contracts, edge-case rules, tests or oracles, and compatibility boundaries.

When a Kyokai rule is written in the normative spec, that spec text is the authority for the rule. Until a rule is extracted, `kyokaidecided.md` is the accepted public shape, public PRs/MRs carry live proposals, and `Kyokaishape.md` acts as an archive and historical index. `phase.md` gives implementation order; it does not define language semantics. If implementation behavior disagrees with the normative spec, the disagreement is a bug in the implementation, the spec, or both, and must be resolved explicitly.
