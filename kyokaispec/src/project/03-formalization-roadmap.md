...
This project chapter records the formalization plan and current public evidence. Kyokai's sequential ownership core has a Gate-B `paper-proven` theorem for the narrow `lambda_K-seq` boundary. Mechanization remains scheduled after self-hosting.
...
Until a theorem is discharged, spec text may say language contract, design goal, or intended invariant. It must not claim that a paper or mechanized proof exists.

Public claims record two separate axes. The D477 semantic-scope axis is `SafeCore`, `SafeConcurrent`, `SafeFFIWrapped`, `UnsafeModule`, or `BackendConforming`, paired with maturity state `designed`, `specified`, `mechanized`, `tested`, or `conformance_checked`. The D312 proof-evidence axis is `intended-by-spec`, `implemented-and-tested`, `conformance-backed`, `paper-proven`, or `mechanically-proven`. A D477 maturity label never implies a D312 proof-evidence label.

A claim names its semantic scope, maturity state, proof-evidence tier, artifact path, assumptions, exclusions, and owner. D479-D487, D502, and D508 also apply: modal words cannot hide implementation freedom, and proof-relevant prose must use explicit contract tables or open a new D-point when accepted shape is incomplete. When proof work changes a rule, the normative chapter and traceability appendix must be updated in the same pass.
...
## Bidirectional Specification And Calculus Correspondence

Every calculus rule, abstraction, lemma family, executable trace, and Lean
theorem identifies the normative clauses and surface, core, or IR facts it
represents or excludes. Every semantic specification change receives a
proof-impact record covering mappings, definitions, lemmas, model traces, Lean
artifacts, compiler IR, and claim tier. A model clarification is resolved as a
normative repair, an accepted model-only abstraction in `deviation.md`, or a
pending D-point; source meaning cannot remain proof-only.

Correspondence records bind source revision, clause IDs, calculus revision,
compiler IR schema, assumptions, trusted base, exclusions, reviewer, and
disposition. Impact classes are `NO_SEMANTIC_IMPACT`, `MAPPING_ONLY`,
`MODEL_AFFECTING`, `PROOF_AFFECTING`, and `CLAIM_INVALIDATING`; each class names
its minimum artifacts and reviewer role. Machine registries generate forward
and reverse views across clauses, calculus rules, theorems, IR schemas, traces,
conformance, and ProofTrace. Human review decides the relationship.

`kyokaicalculus/findings-divergence.md` is the public intake ledger. A finding
closes only after every selected spec, calculus, proof, compiler, test, and claim
artifact is updated and a named reviewer records its disposition. Unknown
correspondence blocks the affected claim, Gate-A closure, or release claim, not
unrelated compiler work.

## Revision-Bound Proof Review

Every `paper-proven` claim names an immutable review packet bound to source
commit or tree digest, theorem IDs, proof revision, model and Lean revisions,
and verification commands. It records the reviewer's display name or stable
public identity, authorship relationship, relevant competence, date, scope,
checklist, findings, severity, author response, repair revision, unresolved
assumptions, exclusions, review class, and disposition.

`maintainer`, `AI`, `community`, or `independent` without an identified reviewer
is incomplete evidence. Author review, AI-assisted review, community review,
and independent qualified-human review remain distinct labels. A material
proof, calculus, or spec change invalidates or narrows the old packet according
to its impact record and requires a new packet or explicit carry-forward.
Missing review evidence downgrades the public claim; it does not refute the
theorem. ProofTrace checks packet identity and coverage, not mathematical
correctness.

> Trace: D580-D581
> Covers: Proof claims carry reviewable two-way semantic correspondence and named revision-bound review evidence.

## Calculus Model Identity And Coupled Review

Every calculus family declares a stable family ID, model epoch, monotonic
revision, source digest, accepted-spec cutoff, and trusted base. Rules,
judgments, lemmas, theorems, traces, executable models, and proof-assistant
artifacts have stable IDs and revisions. An ID is never reused for a different
statement. An incompatible change to representation, judgment form, transition
relation, trusted base, or theorem interpretation advances the model epoch.

A proof review packet binds the exact statement, dependency revisions,
commands, results, source/spec cutoffs, and named human reviewer. Pull requests
that touch this boundary declare one class: `spec_only`, `calculus_only`,
`coupled_semantic`, or `proof_review`. Applicable checks cover registries,
spec-calculus correspondence, executable models, Lean, ProofTrace, clause
extraction, generated views, and review-packet presence.

A `calculus_only` change cannot alter the meaning of a source-language claim.
If it does, the change becomes `coupled_semantic` and cites or opens a D-point.
Public issue forms can record semantic/spec findings, calculus/proof defects,
correspondence drift, and proposals. A complete typed finding in the PR does
not need a duplicate issue.

The release knowledge manifest pins the exact calculus and proof artifacts and
their claim tiers. Monthly and release specification reviews cover the formal
delta since the preceding reviewed knowledge state.

> Trace: D628, D630
> Covers: Formal evidence has stable semantic identity and a Git review class rather than relying on filenames or branch state.
