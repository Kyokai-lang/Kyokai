...
let validate_shebangs mode source =
  ...
  let rec loop offset line_start =
    if offset >= length then Ok ()
      ...
      if offset = 0 && mode = ExecutableEntryBody then skip_first_line offset
...
let render_error error =
  let description =
    ...
    | DisallowedShebang -> "A shebang is legal only at byte zero of an executable-entry .kai source."
  in
