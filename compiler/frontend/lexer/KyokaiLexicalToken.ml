...
let keywords = [
  "additional_invariant"; "alias"; "and"; "as"; "audit"; "band"; "below"; "bit";
  "bitrecord"; "bits"; "bnot"; "body"; "bor"; "borrow"; "break"; "build"; "bxor";
  "capability"; "case"; "compile_error"; "comptime"; "constant"; "continue";
  "contract"; "covers"; "debug"; "default"; "defer"; "do"; "drop"; "else"; "ensure"; "Err";
  "errdefer"; "esac"; "extern"; "false"; "fi"; "field"; "fn"; "for";
  "foreign"; "from"; "function"; "generator"; "if"; "import"; "in"; "instance";
  "internal"; "is"; "join"; "let"; "method"; "module";
  "module_invariant"; "mon"; "nil"; "None"; "not"; "od"; "of"; "Ok"; "or"; "packed";
  "panic"; "pick"; "pragma"; "private"; "produce"; "public"; "qed"; "receiver"; "record";
  "require"; "reserved"; "return"; "rotl"; "rotr"; "seal"; "select"; "shl"; "shr";
  "Some"; "spawn"; "spec"; "static"; "static_assert"; "target"; "taskgroup"; "then";
  "timeout"; "to"; "todo"; "true"; "type"; "typeclass"; "union";
  "opaque"; "unreachable"; "unsafe"; "var"; "wait"; "wake"; "when"; "where"; "while"; "with"; "yield";
]
...
let word_borrow_operators = ["&reborrow"; "&write"; "&read"]
let inherited_forms = ["/="; "--"; "#x"; "#b"; "#o"; "/*"; "|>"; "&~"]
let platform_char_literal_prefixes = ["u8'"; "u'"; "U'"; "L'"]
...
let accepted_word_borrow_operator scanner =
  let exact_operator operator =
    if not (starts_with scanner operator) then false
    else
      let next = scanner.offset + String.length operator in
      next >= scanner.length || not (is_identifier_continue scanner.source.[next])
  in
  List.find_opt exact_operator word_borrow_operators
...
let rec scan_all scanner tokens =
  if at_end scanner then
    ...
    | Some _ ->
       begin match first_prefix scanner platform_char_literal_prefixes with
       | Some inherited ->
       ...
       | None ->
          ...
          | Some _ ->
             begin match accepted_word_borrow_operator scanner with
             | Some operator ->
                begin match scan_symbol scanner start operator with
                | Ok token -> scan_all scanner (token :: tokens)
                | Error error -> Error error
                end
             | None ->
...
let render_error error =
  let description =
    ...
    | DisallowedShebang -> "A shebang is legal only for an executable-entry .kyo source."
